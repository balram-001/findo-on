...
TARGET_TOTAL = 100
FIXED_INDUSTRY = "Pharmaceuticals & Healthcare Manufacturing"

# Additional Safe Industrial Queries for Remaining Leads
ADDITIONAL_QUERIES = [
    ("Herbal & Natural Healthcare", "ayurvedic formulation manufacturing unit Sanwer Road Indore"),
    ("Healthcare Packaging & Containers", "medical packaging container manufacturer Indore"),
    ("Nutritional Supplements", "nutraceutical healthcare formulations manufacturing Indore"),
    ("Surgical & Medical Disposables", "medical hospital disposables manufacturing company Indore"),
    ("Liquid Healthcare Formulations", "syrup suspension healthcare formulation laboratory Indore"),
    ("Healthcare Laboratory Products", "biotech clinical diagnostic products manufacturing Indore"),
    ("Herbal Extraction & Processing", "herbal plant extract manufacturing facility Palda Indore")
]

EXCLUDE_TERMS = [
    "chemist", "medical store", "pharmacy store", "retailer", "trader", "trading", 
    "dealer", "clinic", "hospital", "doctor", "nursing home", "dispensary", 
    "retail shop", "wholesaler", "surgical shop", "medicine shop", "store"
]
...
def search_web_fallback(company_name: str):
    try:
        query = f"{company_name} Indore manufacturing unit contact phone"
...
def complete_to_100():
    ...
    with sync_playwright() as p:
        ...
        for category_name, query_str in ADDITIONAL_QUERIES:
            ...
            for card in cards:
                ...
                try:
                    ...
                    print(f"    🏷️ {category_name} | 📞 {final_phone} ({num_type}) | 📍 {location_text}\n")

                except Exception:
                    continue

        browser.close()
        print(f"\n🎉 Exact 100 leads target completed for {FIXED_INDUSTRY}!")
