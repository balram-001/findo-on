...
# ==========================================
# 2. FOOD PROCESSING & AGRO TARGET QUERIES
# ==========================================
INDUSTRY = "Food Processing & Agro Manufacturing"
TARGET_NEW_LEADS = 50

FOOD_AGRO_QUERIES = [
    # Soya, Oil Refineries & Grain Mills (Dewas, Pithampur, Indore)
    {"query": "Soya solvent extraction plant Dewas Industrial Area", "category": "Soya & Grain Processing"},
    {"query": "Edible oil refinery mill Pithampur Sector 1", "category": "Edible Oil & Fats"},
    {"query": "Automatic roller flour mill Atta Maida Palda Indore", "category": "Soya & Grain Processing"},
    {"query": "Soya food products manufacturer Sanwer Road Indore", "category": "Soya & Grain Processing"},
    {"query": "Edible oil extraction plant Sanwer Road Sector A Indore", "category": "Edible Oil & Fats"},

    # Spices, Snacks, Bakery & Confectionery Plants
    {"query": "Spices masala processing factory Palda Industrial Area Indore", "category": "Spices & Food Ingredients"},
    {"query": "Namkeen snacks manufacturing plant Sanwer Road Indore", "category": "Snacks & Confectionery"},
    {"query": "Industrial bakery biscuits factory Pithampur Sector 3", "category": "Snacks & Confectionery"},
    {"query": "Poha manufacturing mill Palda Indore", "category": "Soya & Grain Processing"},
    {"query": "Confectionery candy factory Polo Ground Indore", "category": "Snacks & Confectionery"},

    # Agro Processing, Dairy & Cold Storage Plants
    {"query": "Agro processing unit Bardari Industrial Area Indore", "category": "Agro Processing & Cold Chain"},
    {"query": "Food processing plant Rau Pigdamber Indore", "category": "Agro Processing & Cold Chain"},
    {"query": "Commercial dairy milk processing plant Pithampur", "category": "Agro Processing & Cold Chain"},
    {"query": "Industrial cold storage frozen food plant Indore", "category": "Agro Processing & Cold Chain"}
]

random.shuffle(FOOD_AGRO_QUERIES)

# Strict Rejects for B2C Retail Shops, Restaurants & Bakeries
STRICT_REJECTS = [
    "restaurant", "sweet shop", "cafe", "hotel", "retail store", "grocery", "kirana",
    "bakery shop", "fast food", "dhaba", "supermarket", "medical store", "chemist", "dawa bazar"
]
...
def is_unwanted_retail(name: str, text: str = "") -> bool:
...
# 3. SCRAPING ENGINE
# ==========================================
async def scrape_food_agro_leads():
    ...
    async with async_playwright() as p:
        ...
        for q_data in FOOD_AGRO_QUERIES:
            if inserted_leads_count >= TARGET_NEW_LEADS:
                break

            query = q_data["query"]
            category = q_data["category"]
            print(f"🔎 [SEARCHING FOOD & AGRO] '{query}'")
            ...
            for card in cards:
                ...
                try:
                    ...
                    if is_unwanted_retail(comp_name):
                        print(f"    ❌ [SKIP RETAIL/RESTAURANT] {comp_name}")
                        continue
                    ...
                    if is_unwanted_retail(comp_name, panel_text):
                        print(f"    ❌ [SKIP RETAIL/RESTAURANT] {comp_name}")
                        continue
                    ...
                    address = "Indore / Pithampur / Dewas Agro Belt"
                    ...
                    print("\n" + "=" * 60)
                    print(f"🚀 [FOOD/AGRO STORED #{inserted_leads_count}] {comp_name}")
    ...
    print(f"\n🎉 Food & Agro Scrape Complete! Total {inserted_leads_count} unique leads inserted.")

if __name__ == "__main__":
    asyncio.run(scrape_food_agro_leads())